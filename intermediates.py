...
def update_int(tau,t2,I_vv,I_oo,Ioooo,Iovvo,Iovvo_2,Iovov):
  ...
  I_oo += 2*np.einsum('cdkl,ilcd->ik',twoelecint_mo[occ:nao,occ:nao,:occ,:occ],tau) 
  I_oo+= -np.einsum('dckl,lidc->ik',twoelecint_mo[occ:nao,occ:nao,:occ,:occ],tau) 
...
def singles_intermediates(t1,t2,I_oo,I_vv,I2):
  ...
  #Iovvo_3=np.zeros((occ,virt,virt,occ))
